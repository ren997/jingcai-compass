...
/**
 * 修复 V17 遗留外部待复核身份清理：仅移除可由来源名称键唯一复算的无引用临时实体。
 */
public class V20__RepairLegacyPendingIdentityCleanup extends BaseJavaMigration {
    ...
    private void cleanupEntities(Connection connection, EntityType entityType) throws SQLException {
        ...
        // 2) 以旧 NAME/SCOPED_NAME 哈希和当前规范化规则确定唯一候选，拒绝模糊匹配。
        ...
        // 3) 删除前再次由 SQL 保护比赛、别名和其它映射引用，竞争写入时宁可保留。
        for (LegacyMapping mapping : mappings) {
    ...
    private List<LegacyMapping> loadLegacyMappings(Connection connection, EntityType entityType)
        ...
        String sql = """
                ...
                WHERE provider_code = ?
                  ...
                  AND mapping_method = ?
                  AND %s IS NULL
                """.formatted(entityType.externalIdColumn(), entityType.mappingTable(), entityType.entityIdColumn());
    ...
    private List<InternalEntity> loadOrphanCandidates(Connection connection, EntityType entityType)
            throws SQLException {
        String sql = entityType.orphanCandidateSql();
    ...
    private boolean matches(LegacyMapping mapping, InternalEntity candidate) {
        ...
        for (String name : candidate.names()) {
            if (name == null || NameNormalizationSupport.normalizedKey(name).isEmpty()) {
                continue;
            }
