...
class AdminPredictionStatusControllerSecurityTest {
    ...
    @Test
    void statusPathsRequireAdministratorJwt() throws Exception {
        mockMvc.perform(post("/api/admin/prediction-status/locks/list")
                        .contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isUnauthorized()).andExpect(jsonPath("$.code").value(ErrorCode.AUTH_UNAUTHORIZED.code()));
        ...
        mockMvc.perform(post("/api/admin/prediction-status/settlements/list")
                        .header("Authorization", "Bearer viewer-token").contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isForbidden()).andExpect(jsonPath("$.code").value(ErrorCode.ACCESS_DENIED.code()));
    }

    @Test
    void administratorCanReadStatusPath() throws Exception {
        when(jwtDecoder.decode("admin-token")).thenReturn(jwt("ADMIN"));
        when(queryService.locks(any())).thenReturn(new AdminPredictionStatusPageVo(List.of(), 1, 20, 0, 0));

        mockMvc.perform(post("/api/admin/prediction-status/locks/list")
                        .header("Authorization", "Bearer admin-token").contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.code").value(ErrorCode.SUCCESS.code()));
    }
