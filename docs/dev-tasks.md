...
| M2 标准化与映射 | `IN_PROGRESS` | T203 已完成；下一任务 T204 |
...
当前没有 `IN_PROGRESS` 任务。下一次开发优先进入：`T204 双源比赛自动映射`。

1. 将文档顶部“当前活动任务”改为 T204，并把 T204 从 `TODO` 改为 `IN_PROGRESS`。
2. 实现比赛级自动映射与待复核队列。
3. 运行 T204 验证命令并回写状态。
