...
- 执行步骤：
  - [ ] 定义人工赛果事实来源、必填来源说明/补录原因、正常完赛/取消等可录入状态及比分校验；不得把人工输入伪装为官方原始载荷。
  - [ ] 实现 JWT 保护、二次确认和开赛后条件校验的管理员补录接口；任何请求不得直接创建或修改 Settlement。
  - [ ] 复用既有不可变事实版本与审计：首次补录追加事实，重复相同输入幂等复用，修正追加新事实版本并保留原因与操作者。
  - [ ] 在事实成功写入后仅调用既有 T404/T405 结算与重算流程；失败必须可追溯且不得留下半完成的事实/结算组合。
  - [ ] 增加后台录入、事实版本链和“人工补录，非官方源”提示；不在公共页面把人工补录描述为官方赛果。
  - [ ] 覆盖鉴权、字段/状态/比分校验、二次确认、幂等、事实修正、结算重算、并发和 traceId；在 PostgreSQL 16 验证事实与结算不可覆盖约束。
- 验证命令：
