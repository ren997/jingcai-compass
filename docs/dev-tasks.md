...
- 当前活动任务：无
- 下一任务：`T104 原始响应入库与同步运行服务`
- 最近完成增量：`T103 Stub 双数据源`
...
说明：此前为验证产品和数据源提前完成了比赛列表纵向切片，因此 T106、T501、T503 当前为 `PARTIAL`。这些增量不改变主线依赖；T002 已补齐，T003 经项目负责人决定跳过，恢复正式开发时从 T004 继续补齐底座。
...
| M1 Provider 基础 | `PARTIAL` | T101～T103 已完成；下一步 T104 原始入库与同步运行 |
...
当前没有 `IN_PROGRESS` 任务。下一次开发按以下步骤执行 `T104 原始响应入库与同步运行服务`：

1. 将文档顶部“当前活动任务”改为 T104，并把 T104 从 `TODO` 改为 `IN_PROGRESS`。
2. 实现同步运行状态机与原始响应入库（哈希、JSONB、解析错误保留）。
3. 固定 Provider 调用模板：运行 -> 请求 -> 原始入库 -> 解析 -> 完成。
4. 补齐重复响应、单条失败、整批失败与恢复测试。
5. 运行 T104 验证命令并回写状态。
