...
| 里程碑 | 状态 | 说明 |
| --- | --- | --- |
| M0 工程基线 | `PARTIAL` | T002 已完成；T003 因不安装本地 Docker 跳过，下一步执行 T004 |
...
当前没有 `IN_PROGRESS` 任务。T003 已按项目负责人决定跳过，下一次开发按以下步骤执行 `T004 公共后端基础设施`：

1. 将文档顶部“当前活动任务”改为 T004，并把 T004 从 `TODO` 改为 `IN_PROGRESS`。
2. 定义统一响应、分页结果和错误码模型。
3. 实现全局异常处理与 traceId Filter。
4. 配置 MyBatis-Plus 分页、公共审计字段和安全边界。
5. 补充响应、异常、traceId、安全和配置测试。
6. 运行 T004 验证命令及通用检查并回写状态。
