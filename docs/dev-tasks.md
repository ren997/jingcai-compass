...
- 当前活动任务：`T602 后台同步与映射复核页面`
- 下一任务：`T603 基础指标、日志与数据源告警`
- 最近完成增量：`T505 首页汇总（T602 进行中）`
...
当前无活动任务；T401 已完成赛果事实与结算持久化基线及 PostgreSQL 16 CI 验证，T402 已完成 Stub 驱动的赛果同步，T403 已完成纯函数市场结算规则，T404 已完成自动结算闭环，T405 已完成赛果修正后的版本化结算重算，T501 已完成公共持久化比赛查询，T504 已完成历史与统计前后端，T505 已完成首页汇总，T506 已完成公开预测详情前后端增量，T507 已完成公开历史与统计 API，M4/M5 已完成。

下一主线任务为 `T602 后台同步与映射复核页面`；在既有管理员鉴权基础上展示同步运行、额度、错误和待复核映射，并对确认/拒绝操作保持权限、二次确认与追加式审计。
