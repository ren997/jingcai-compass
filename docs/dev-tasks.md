...
- 当前活动任务：无
- 下一任务：`T203 联赛与球队标准化`（T106/T107 为连续观测，可与主线穿插）
- 最近完成增量：`T202 体彩比赛池同步`
...
| M2 标准化与映射 | `TODO` | T202 已完成；下一任务 T203 |
...
当前没有 `IN_PROGRESS` 任务。下一次开发优先进入：`T203 联赛与球队标准化`。

1. 将文档顶部“当前活动任务”改为 T203，并把 T203 从 `TODO` 改为 `IN_PROGRESS`。
2. 实现联赛/球队名称规范化与供应商映射优先规则。
3. 未知名称只建候选，不自动合并。
4. 运行 T203 验证命令并回写状态。
