...
- 当前活动任务：`T207 双源同步编排闭环`
- 下一任务：`T301 预测与快照 migration`
...
- 执行步骤：
  - [ ] 固定一批包含正常、别名、未知名称、映射冲突和缺盘场景的 Stub 业务日样本。
  - [ ] 体彩比赛写库后调用联赛/球队标准化服务，幂等回填 `league_id`、`home_team_id`、`away_team_id`。
  - [ ] 未确认标准实体只进入待处理队列，不通过模糊匹配静默合并。
  - [ ] 亚盘同步优先复用标准实体和已确认比赛映射，低置信度数据继续由 T205 复核。
  - [ ] 增加已有 `matches` 的受控回填入口，支持按业务日重跑且不覆盖人工确认结果。
  - [ ] 串通 raw payload → matches → 标准实体 → match mapping → asian odds snapshot 全链路。
  - [ ] 输出比赛数、标准化完成数、待处理数、映射确认数、有效盘口数和覆盖率。
  - [ ] 覆盖重复运行、单场失败、事务边界和人工确认结果复用测试。
- 验证命令：
...
- 恢复入口：先补端到端 Stub fixture 和失败场景，再接同步编排，避免直接对真实 Provider 调整流程。
- 执行记录：
...
- 执行步骤：
  - [ ] 固定完整预测契约：三项概率、让球胜平负倾向、预期总进球、置信等级、分析摘要、模型/特征版本和生成批次。
  - [ ] 定义预测、预测版本、公开快照和存储对象元数据字段及可空规则。
  - [ ] 编写 V7 migration；保留已执行的 V1～V6 原样，不重命名、删除或修改。
  - [ ] 增加概率范围与概率和、状态、版本、哈希、发布时间和锁定时间约束。
  - [ ] 设计同一比赛/模型多版本唯一约束和当前版本查询索引。
  - [ ] 创建 Entity、Mapper、`PredictionStatusEnum` 和快照状态枚举。
...
- 交付物：
  ...
  - 离线模型样例文件
- 执行步骤：
  - [ ] 定义包含比赛、模型/特征版本、三项概率、让球倾向、预期总进球、置信等级、分析摘要和生成时间的 `PredictionImportDto`。
  - [ ] 校验比赛存在、未开赛、概率在区间内且概率和满足精度要求。
  - [ ] 校验枚举、数值精度、摘要长度和禁止收益承诺等合规表达。
  - [ ] 规范化小数精度，禁止以字符串或百分数字段混用。
  - [ ] 实现整批校验后写入，任何失败不产生半批数据。
  - [ ] 保存模型输入文件或批次哈希，支持重复导入幂等。
  - [ ] P0 先完成结构化文件导入；受控模型命令入口保留为同一 Dto 的可替换输入适配器，不作为 T302 完成前提。
  - [ ] 编写边界概率、概率和错误、非法文案、已开赛和重复批次测试。
- 验证命令：
...
- 执行步骤：
  ...
  - [ ] 使用 V8 创建管理员账号和必要安全字段；保留 V1～V7 原样，不开放普通用户注册。
