...
| M2 标准化与映射 | `IN_PROGRESS` | T204 已完成；下一任务 T205 |
...
当前没有 `IN_PROGRESS` 任务。下一次开发优先进入：`T205 映射人工复核接口`。

1. 将文档顶部“当前活动任务”改为 T205，并把 T205 从 `TODO` 改为 `IN_PROGRESS`。
2. 实现映射列表/详情/确认/拒绝 API 与审计。
3. 运行 T205 验证命令并回写状态。
