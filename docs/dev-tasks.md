...
T401 + T404 + T405 + T601 -> T406（受控人工赛果补录）
...
- T306 使用当前可拉取的比赛及已确认映射的盘口完成首版离线预测，不等待 T106 的真实赛果契约或 T209 的真实库清理证据；缺失或未确认的特征必须跳过并说明原因。T406 仅补录不可变赛果事实、复用现有自动结算，不改变“结算结果不可人工编辑”的规则。
...
| M3 预测生成、发布与快照 | `PARTIAL` | T301～T306、T601 已完成预测承载、可解释离线生成、发布、锁定和确定性公开快照；T610 已完成本地亚盘让球/大小球方向与生成快照可追溯性增量，待完整 CI / PR 交付 |
| M4 赛果与结算 | `DONE` | T401～T406 已完成不可变赛果、自动结算/修正重算及受控人工赛果补录；人工事实仅用于开发验证和模型评估，不替代官方数据源 |
| M5 公共 API 与前端 | `DONE` | T501、T502、T503、T504、T505、T506、T507、T609 已完成；公开比赛、预测、历史、统计与首页闭环均由持久化事实查询支撑 |
...
- 执行步骤：
  ...
  - [ ] 接入并核对真实体彩官方赛果接口，覆盖正常完赛：为 `ChinaSportteryProvider.fetchMatchResultsRaw(startDate, endDate)` 实现经验证的赛果请求、原始响应存档与显式 Dto 解析；先用 2026-07-29 等已完赛竞彩日验证请求参数、比赛编号、比分、最终状态和供应商更新时间，禁止以 Stub、页面人工抄录或未验证候选地址替代。
  - [ ] 收集延期、取消、改期和官方修正案例。
...
- 恢复入口：先完成真实 `CHINA_SPORTTERY/SPORTTERY_RESULT` Provider 接入。当前实现会返回“赛果接口尚未完成验证”；应从 `docs/data-sources.md` 记录的候选 `/gateway/jc/football/getMatchResultV1.qry` 开始，以真实已完赛竞彩日验证请求与响应后，再建立自动采集与同步运行记录；授权调查可同步进行。
- 执行记录：
  - 2026-07-22：完成首次官方页面、请求 URL、真实响应和页面数据核对；连续观测尚未开始。
  - 2026-07-30：项目负责人授权本机测试数据环境手工同步 2026-07-29～2026-07-30 赛果。`MatchResultSyncJob` 已正常调度并建立 `data_sync_runs` 运行 ID 8，但 `CHINA_SPORTTERY/SPORTTERY_RESULT` 在 Provider 层返回“赛果接口尚未完成验证”，未访问上游、未写入赛果事实；常规 local 服务已恢复为任务关闭状态。
  - 2026-07-30：开始“真实赛果 Provider + 管理员手动触发 + 单次验证”增量。先关闭本地 8080/8081 与 Vite 5173；只接受官网公开、无 Cookie/Authorization/浏览器令牌依赖的请求。实现范围包括 raw 原样存档后由 Provider 解析、默认上海昨日/最多连续 7 天的管理员手动同步、与定时任务共享的单进程协调器及同步运行页操作区；不自动结算、不启动 14 天观测、不执行 PostgreSQL 集成测试、CI、推送或 PR。计划运行 `npm run backend:test`、`cd frontend && npm run test && npm run build`、`git diff --check`，最后用关闭定时任务的 local profile 对昨天执行一次受控手动冒烟。
  - 2026-07-30：本轮本地增量完成。官网候选 `GET /gateway/jc/football/getMatchResultV1.qry` 以公开头白名单、无 Cookie/Authorization/浏览器令牌调用返回 HTTP 403，无法确认日期参数、字段、最终/作废/修正状态或供应商更新时间，因此 `ChinaSportteryProvider` 继续受控失败，不接入猜测契约。已增加 raw 存档后委派 Provider 解析的接口、单进程同步协调器、管理员手动同步 API 与后台操作区；默认上海昨日，显式窗口限制 1～7 天且不允许未来日期，返回安全运行摘要而不暴露 raw，且不自动结算。local profile（任务关闭）健康检查通过，管理员手动同步 2026-07-29 创建运行 ID 9，返回 `SUCCESS` API 包络内的已完成 `FAILED` 运行（0 拉取/0 解析失败），未写入赛果事实；服务已关闭。
- 完成标准：
  - 比赛池字段完整率 100%。
  - 正常完赛场次赛果可获取率 100%。
  - 延期、取消和修正场景有记录。
...
- 验证记录：
  ...
  - 2026-07-30：手工赛果同步运行 ID 8 为 `FAILED`，请求窗口 2026-07-29～2026-07-30，0 成功/0 失败解析项、0 credits；后台同步运行页和受控日志均确认失败原因。该失败是未实现真实赛果接口的证据，不计入 14 天连续观测。
  - 2026-07-30：`mvn -f backend/pom.xml clean test` 通过（437 项）；`cd frontend && npm run test && npm run build` 通过（13 个文件、64 项）。“无会话公开请求”候选赛果端点 HTTP 403 与 local 运行 ID 9 已记入 `data-sources.md`；未运行集成测试、CI，未推送或建 PR。待官方提供可验证的公开赛果请求契约后，先实现真实 Dto 解析与 raw 归档冒烟，再考虑启动 14 天观测。
...
- 执行步骤：
  ...
  - [x] 将 The Odds API 请求扩展为 `spreads,totals`，严格解析大小球成对盘口，并以日职两场受控同步验证快照写入。
...
- 执行记录：
  ...
  - 2026-08-07：开始大小球受控增量；范围限于 The Odds API 的 `spreads,totals` 请求与严格成对解析、现有快照字段写入、日职两场重同步和盘口复核。不改数据模型、预测契约、赛果同步或连续观测。计划执行 The Odds Provider/亚盘同步专项测试、后端全量测试、前端构建和 `git diff --check`。
  - 2026-08-07：完成受控增量。The Odds API 按 `spreads,totals` 拉取日职两场，严格要求 Over/Under 同线且完整；已确认的两条赛事映射复用。同步运行 ID 13 写入 12 条包含大小球的快照；最终运行 ID 14 再次成功，因盘口变化新增 3 条快照。两场均取得 6 家公司的大小球线；本次不构成连续观测或 90% 覆盖率证据，任务保持 `PARTIAL`。
...
- 验证记录：
  ...
  - 2026-08-07：`TheOddsApiProviderTest` 5 项通过，覆盖 `spreads,totals` 查询、完整大小球写入和大小球盘口线不一致失败；`npm run backend:test` 439 项通过；前端 Vitest 13 个文件、64 项通过，生产构建通过（前端未改动）；受控 real local 同步运行 ID 13/14 均成功，最终公开详情可读取两场的 `totalLine`、`overOdds` 和 `underOdds`。
...
- 执行记录：
  ...
  - 2026-07-30：时效修正的前后端本地增量已提交；项目负责人要求优先处理 T209，且本轮不补 PostgreSQL 集成测试，T208 保持 `PARTIAL`。
-- 验证记录：
...
- 依赖：T208
- 交付物：
  - 不创建内部临时联赛/球队的 The Odds 待复核身份模型与安全数据清理迁移
  - 基于已确认 Provider 联赛、作用域球队关系的自动赛事映射复用
  - 明确区分“外部待复核身份”与“竞彩内部标准实体”的后台复核页面
- 执行步骤：
  - [x] 新增 Flyway migration，使 `provider_league_mappings.league_id`、`provider_team_mappings.team_id` 在 `PENDING`/`REJECTED` 外部身份阶段可为空；保留已确认关系的外键、状态约束、审计及唯一性。
  - [x] 调整联赛与球队标准化：The Odds 新外部身份只保存 Provider、外部 ID、展示名、规范化键、`sport_key` 作用域与 `PENDING`，不在 `leagues`/`teams` 创建同名临时实体；仅确认到已存在的竞彩内部实体时写入实体 ID 与 `MANUAL_CONFIRMED`。
  - [x] 审计并安全处理既有 The Odds `NAME_CANDIDATE` 待复核记录：先解除其临时实体关联；仅删除没有比赛、别名、已确认映射或其他引用的孤立临时实体，任何不满足删除条件的记录保留且标注为待人工处置，禁止从单场赛事关联反推或批量确认。
  - [x] 将球队候选限定为可信的竞彩内部标准实体，排除未确认 Provider 临时实体；页面把“暂存实体”改为“已关联内部实体”，未确认时明确显示“尚未关联”。
...
- 依赖：T207、T302
- 交付物：
  - `prediction/service` 下的可解释离线预测生成器与特征快照模型
  - 生成 `PredictionImportDto` 兼容 JSON 批次的受控命令入口
  - 模型版本、特征版本、输入可用性和跳过原因的执行日志
- 执行步骤：
  - [x] 固定 v1 范围：仅使用已持久化的竞彩比赛、体彩让球/SP 与已确认映射的亚洲让球/大小球；未确认映射、缺少必要特征或已开赛比赛不得生成预测，并输出稳定跳过原因。
  - [x] 定义可复现的特征快照与输入版本：同一持久化输入、模型版本和生成时间规则必须得到相同的预测批次内容；不从 Controller 请求路径调用 Provider。
  - [x] 实现赔率/让球/大小球驱动的可解释基线，生成主胜/平局/客胜概率、让球胜平负倾向、预期总进球、置信等级和不承诺收益的分析摘要；概率和与精度必须满足既有导入契约。
  - [x] 提供离线命令或受控任务入口，按业务日生成严格的 UTF-8 JSON 批次；生成只产生 DRAFT 导入输入，仍由既有管理员发布、锁定和快照流程决定是否公开。
  - [x] 写入低基数执行指标与结构化日志，记录输入比赛数、生成数、跳过分类、模型/特征版本和批次哈希，且不输出密钥或原始 Provider 载荷。
  - [x] 补齐单元测试：确定性生成、概率边界/和、主客方向、盘口缺失、未确认映射、已开赛拒绝、批次可由 T302 导入及不会自动发布。
  - [x] 在 PostgreSQL 16 集成验证中使用持久化比赛和盘口快照验证特征选择、幂等批次、导入后的发布/锁定兼容性与并发安全；真实赛果仅用于后续评估，不作为本任务前置。
- 验证命令：

  ```bash
  mvn -B -ntp -f backend/pom.xml -Dtest=*BaselinePrediction*Test,*PredictionImport*Test test
  mvn -B -ntp -f backend/pom.xml test
  mvn -B -ntp -f backend/pom.xml -Pintegration verify
  git diff --check
  ```
...
- 完成标准：
  - 对符合输入门槛的未开赛比赛可稳定生成可导入的完整预测批次；输入不完整时安全跳过而非编造特征。
  - 输出可复现、通过 T302 导入校验，且不会绕过现有的发布、锁定、快照与审计规则。
  - 模型明确标识为首版可解释基线，不把无赛果样本宣传为训练、回测或收益证明。
  - 普通测试、PostgreSQL 16 集成验证和 GitHub Actions 均通过。
- 恢复入口：从当前已持久化的比赛、体彩盘口和已确认的亚洲盘口快照审计最小可用特征集；先创建 `codex/t306-baseline-model` 分支，再实现生成器。
- 执行记录：
...
- 交付物：
  - 管理员受保护的人工赛果补录 Dto、Vo、Service 与后台操作页
  - 追加式人工赛果事实、来源说明、操作审计与修正重算联动
  - 人工补录与官方赛果来源的明确展示与风险提示
- 执行步骤：
  - [x] 定义人工赛果事实来源、必填来源说明/补录原因、正常完赛/取消等可录入状态及比分校验；不得把人工输入伪装为官方原始载荷。
  - [x] 实现 JWT 保护、二次确认和开赛后条件校验的管理员补录接口；任何请求不得直接创建或修改 Settlement。
  - [x] 复用既有不可变事实版本与审计：首次补录追加事实，重复相同输入幂等复用，修正追加新事实版本并保留原因与操作者。
  - [x] 在事实成功写入后仅调用既有 T404/T405 结算与重算流程；失败必须可追溯且不得留下半完成的事实/结算组合。
  - [x] 增加后台录入、事实版本链和“人工补录，非官方源”提示；不在公共页面把人工补录描述为官方赛果。
  - [x] 覆盖鉴权、字段/状态/比分校验、二次确认、幂等、事实修正、结算重算、并发和 traceId；在 PostgreSQL 16 验证事实与结算不可覆盖约束。
- 验证命令：

  ```bash
  mvn -B -ntp -f backend/pom.xml -Dtest=*ManualResult*Test,*Settlement*Test test
  mvn -B -ntp -f backend/pom.xml test
  cd frontend && npm run test && npm run build
  mvn -B -ntp -f backend/pom.xml -Pintegration verify
  git diff --check
...
- 完成标准：
  - 管理员能以可审计方式补录已核实赛果事实；不得直接编辑结算结果或覆盖历史事实。
  - 人工赛果修正自动沿用既有版本化重算；事实、结算、审计和公开修正标识保持一致。
  - 页面明确人工来源及其非生产数据源属性；T106/T108 的真实源验证和生产 Go/No-Go 标准不因此降低。
  - 普通测试、PostgreSQL 16 集成验证和 GitHub Actions 均通过。
- 恢复入口：T306 完成后，先核对现有 `match_result_facts`、`SettlementService`、T405 重算入口和后台 JWT/二次确认组件，设计最小的人工事实输入契约；不以 SQL 或数据库后台直接写入代替应用流程。
- 执行记录：
...
- 状态：`DONE`
- 优先级：P0
- 依赖：T206、T301、T303、T501、T503
- 交付物：
  - 公开比赛分页项的体彩市场、亚盘主盘和已发布预测摘要
  - 当日比赛卡片的赔率和预测展示
- 执行步骤：
  - [x] 定义明确公开列表 Vo；从持久化最新体彩快照、已确认映射的完整亚盘主盘和当前公开预测批量装配，绝不返回 `DRAFT` 或调用 Provider。
  - [x] 保持分页查询无 N+1，补齐无市场、无主盘、无公开预测和多模型当前预测的后端测试。
  - [x] 在 `/matches` 卡片展示胜平负、让球胜平负、亚盘主盘让球/大小球和预测摘要，并为暂缺数据给出明确状态与窄屏布局。
  - [x] 补齐前端渲染和公开请求契约测试，并在 2026-08-11 已发布样本上进行浏览器验证。
- 验证命令：
...
- 完成标准：
  - 游客无需登录即可在比赛列表直接看到四类摘要；缺失数据不伪造。
  - 草稿预测继续只在管理员草稿页可见，公开列表仅使用当前 `PUBLISHED`/`LOCKED` 版本。
  - 新数据通过分页批量查询取得，自动化测试与真实页面均验证通过。
- 执行记录：
  - 2026-08-11：开始执行。项目负责人要求在公开比赛卡片直接查看胜平负、让球胜平负、亚盘主盘让球/大小球赔率和预测结果。范围为扩展既有公开分页响应和前端卡片；不新增 migration、不调用 Provider、不改发布/锁定/快照规则。计划运行后端与前端全量测试、生产构建、差异检查，并用当天已发布的 8 条预测进行浏览器验证。
  - 2026-08-11：完成。`/api/public/matches/list` 以当前页比赛 ID 批量装配最新体彩 SP、已确认映射下且让球/大小球完整的最新亚盘主盘，以及每个模型的当前 `PUBLISHED`/`LOCKED` 预测；服务与 SQL 双层过滤 `DRAFT`。前端卡片直接显示四类摘要，缺失数据明确为暂缺。重启本地后端后，浏览器实测周二002（阿拉木图 vs 索列夫）显示胜平负 `2.36/2.90/2.77`、让球胜平负 `5.65/3.80/1.45`、`betonlineag` 亚盘 `0.00` 与大小球 `2.00`，以及 `t306-odds-baseline-v1` 的 `37.0%/30.5%/32.4%` 预测；8 条已发布样本均可访问。
- 验证记录：
  - 2026-08-11：专项后端 20 项、前端 30 项与前端生产构建通过；全量 `mvn -B -ntp -f backend/pom.xml test` 通过（463 项），`npm --prefix frontend run test` 通过（15 文件、69 项），`npm --prefix frontend run build` 通过，`git diff --check` 通过。

### T610 亚盘方向预测与标签澄清

- 状态：`PARTIAL`
- 优先级：P0
- 依赖：T206、T302、T303、T306、T503、T608、T609
- 交付物：
  - 明确区分竞彩让球胜平负与预期总进球的公共标签
  - 可追溯的亚盘让球赢盘与大小球方向预测字段、迁移、导入校验和发布哈希
  - 生成时绑定的完整已确认亚盘主盘快照，以及公开/草稿复核展示
  - 亚盘专用 V3 输出：让球与大小球各自的置信等级；低置信方向不进入可发布预测
- 执行步骤：
  - [x] 更新需求、任务看板与公共标签，明确“竞彩让球胜平负：客胜”“预期总进球：2.01”的语义。
  - [x] 新增亚盘让球、大小球方向枚举和预测快照关联；历史 V1 预测保持为空，新模型输出必须完整且不可在锁定后改写。
  - [x] 将方向与亚盘快照纳入导入一致性、内容哈希、快照清单和公开/后台 Vo，保证来源、盘口、赔率和采集时间可复核。
  ...
  - [x] 将竞彩让球结果明确显示为让球胜/平/负及体彩让球线，并在亚盘方向旁显示对应亚盘线；不改变模型输出或已生成数据。
  - [x] 将新模型改为亚盘专用输出，旧版竞彩字段仅为历史兼容而不再作为新模型预测或公开展示。
  - [x] 增加亚盘让球、大小球独立置信等级；低置信方向不生成可发布项，发布时再次校验。
  - [x] 更新公共页、详情和草稿复核为仅展示亚盘方向及对应分项置信等级，并更新自动化测试与迁移验证。
...
- 完成标准：
  - 公共卡片把体彩让球胜平负和预期总进球分别用无歧义标签展示，且历史数据仍可正常显示。
  - 新 V3 亚盘预测仅在完整已确认亚盘主盘存在时产生，记录可发布的亚盘让球/大小球方向、各自中/高置信等级及对应快照；公开与后台复核可查看这些输入，不泄露草稿。
  - 方向不被展示为独立校准概率或正 EV；亚洲盘结算尚未纳入 MVP 结算指标。
  - Flyway PostgreSQL 集成验证与 GitHub Actions 成功后，任务才能标记 `DONE`。
- 执行记录：
  ...
  - 2026-08-12：恢复执行。用户指出“亚盘主队赢盘”与“竞彩让球客胜”会造成错误的同一市场解读；本次仅调整展示和可解释基线文字：竞彩使用“让球胜/平/负”并带体彩官方让球线，亚盘方向带生成快照让球线。不会修改概率、方向选择、批次、预测内容或已生成草稿。
  - 2026-08-12：完成展示澄清增量。草稿、公共比赛卡片和详情现在显示完整的“竞彩让球胜平负（主队 -1）：让球胜/平/负”与“亚盘让球（平手）：主队/客队赢盘”；非零亚盘仍显示主队方向和数值。后端全量 468 项、前端定向 26 项、前端生产构建及差异检查通过。未改模型输出或任何已生成数据；完整 PostgreSQL 集成验证、Draft PR 和 GitHub Actions 仍因 GitHub CLI 未登录而待补，任务保持 `PARTIAL`。
  - 2026-08-12：开始“亚盘专用预测与分项置信度”增量。项目负责人决定去除整场置信度和竞彩预测展示；新模型只产生亚盘让球/大小球方向与各自置信等级。按单市场口径：低置信方向不写入可发布预测，另一方向达到中/高时仍可单独发布；旧版已发布预测、体彩赔率和既有体彩结算统计保持历史兼容，不重写。
  - 2026-08-12：完成 V3 本地增量。新增 V21：`prediction_type=ASIAN` 时体彩概率、竞彩让球、预期总进球和整场置信度必须为空；亚盘让球/大小球分别要求方向与 `MEDIUM/HIGH` 置信度成对存在，`LOW` 不得落库。生成器仅从已确认完整亚盘主盘产生可发布方向，两项均低则跳过整场；公共列表/详情与草稿管理页只呈现亚盘方向、盘口和分项置信度。旧 `SPORTTERY` 预测、赔率和结算链保持历史兼容，亚盘预测不进入已有体彩结算。修复 V21 组合约束语法、NULL 字段批次复用和固定时间 JSON 序列化，避免并发生成误冲突。
  - 2026-08-12：本地验证通过：`mvn -B -ntp -f backend/pom.xml test`（468 项）、`npm run test -- --run`（前端 15 文件、71 项）、`npm run build`、`mvn -B -ntp -f backend/pom.xml -Pintegration '-Dit.test=PostgresApplicationIT,PredictionImportApplicationIT' verify`（12 项，PostgreSQL 16.14，Flyway V1～V21，覆盖 V3 迁移、低置信拒绝、双低跳过、发布不可变与并发复用）以及 `git diff --check`。尚无 GitHub CLI 登录会话，未建立 Draft PR / GitHub Actions；不生成或发布真实今日 V3 预测，任务保持 `PARTIAL`。
  - 2026-08-12：复核完成。恢复同一生成批次必须具有相同原始文件 SHA-256 的校验，并以服务层再次校验亚盘每个已选方向必须绑定 `MEDIUM/HIGH`，防止绕过导入校验。新增 2 项单元覆盖；定向单元 29 项、含并发复用的 PostgreSQL 集成 12 项均通过。未生成或发布真实 V3 预测，任务保持 `PARTIAL`，等待完整 CI / PR 交付。
  - 2026-08-12：项目负责人授权按 V3 重新生成当天草稿。使用 local 配置启动本地后端并登录管理端，`2026-08-12` 候选 3 场，新增 2 条 `t306-asian-baseline-v3` 草稿、复用 0 条，批次 `t306-asian-2026-08-12-2424415beb7a8a0f`；另 1 场因 `MISSING_CONFIRMED_ASIAN_MARKET` 跳过。普拉滕斯 vs 科金博联仅产生“大小球 2.00：小球（中）”，亚盘让球因低置信不写入；帕梅拉斯 vs 波特诺产生“亚盘让球 主 -1.5：客队赢盘（中）”及“大小球 2.50：小球（中）”。本次只生成 DRAFT，不发布。
...
- 状态：`TODO`
- 优先级：P0
- 依赖：T108、T305、T306、T405、T406、T505、T602、T604、T606
- 交付物：
  - 验收报告
  - 连续运行报告
  - 备份恢复记录
...
- 执行步骤：
  ...
  - [ ] 演练 T306 预测生成、预测导入、发布、锁定、快照、T406 人工赛果补录、结算和历史全链路。
...
- 完成标准：
  - 数据源达到 Go 标准。
  - 首版模型生成、发布、锁定、快照、人工/官方赛果、结算和历史闭环通过。
  - 连续运行期间无静默丢数。
  - 数据库备份恢复演练通过。
  - 风险提示、免责声明和隐私说明存在。
...
| 2026-07-30 | T209 | `TODO` 新增为下一任务 | 外部 The Odds `PENDING` 身份不得再新建或伪装为竞彩内部实体；计划以 V17 允许空内部关联、受控清理孤立临时实体，并仅在联赛/主队/客队三个 Provider 作用域关系均经独立人工确认后自动复用赛事映射。全局 `team_aliases` 不增加联赛字段，单场赛事确认不得反推长期别名。 |
