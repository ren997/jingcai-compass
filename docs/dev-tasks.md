...
- 当前活动任务：无
- 下一任务：`T003 Testcontainers 与上下文测试`
- 最近完成增量：`T002 后端依赖与配置分层`
...
2. 确认所有依赖任务为 `DONE`；原型纵向切片必须在任务中明确记录例外，不能当作依赖已经完成。
...
说明：此前为验证产品和数据源提前完成了比赛列表纵向切片，因此 T101、T103、T106、T501、T503 当前为 `PARTIAL`。这些增量不改变主线依赖；T002 已补齐，恢复正式开发时从 T003 继续补齐底座。

## 4. 当前进度

| 里程碑 | 状态 | 说明 |
| --- | --- | --- |
| M0 工程基线 | `PARTIAL` | T002 配置底座已完成，下一步执行 T003 Testcontainers |
...
- 交付物：
  ...
  - M0 最小安全配置
- 执行步骤：
  - [ ] 定义统一 `ApiResponse<T>`、`PageResult<T>` 和错误码枚举。
  - [ ] 实现参数校验、业务异常和未知异常的全局处理。
  - [ ] 增加 traceId Filter，并把 traceId 同步写入日志上下文和响应头。
  - [ ] 配置 MyBatis-Plus 分页上限与公共审计字段处理。
  - [ ] 配置 SpringDoc、Actuator 和 local/prod 暴露策略。
  - [ ] 保留 `/api/public/**` 匿名只读，其余后台路径默认拒绝。
  - [ ] 为响应包装、异常、安全边界和配置编写测试。
- 验证命令：
...
当前没有 `IN_PROGRESS` 任务。下一次开发按以下步骤执行 `T003 Testcontainers 与上下文测试`：

1. 将文档顶部“当前活动任务”改为 T003，并把 T003 从 `TODO` 改为 `IN_PROGRESS`。
2. 在 T003 执行记录中写明 Docker/Testcontainers 范围。
3. 增加 PostgreSQL Testcontainers 和 `application-test.yml`。
4. 创建隔离数据库测试配置，强制测试不得指向云端地址。
5. 让 Flyway 在容器空库执行，并移除上下文测试中的 DataSource/Flyway 临时排除。
6. 运行 T003 验证命令及通用检查并回写状态。

随后严格按以下顺序补齐底座：

```text
T003 -> T004 -> T005 -> 恢复 T101
```

在 T004 `DONE` 前，不继续扩展当前实时比赛列表的业务功能；已经存在的 Provider 和页面只作为经过验证的纵向原型保留。
...
| 2026-07-22 | T002 | `PARTIAL -> DONE` | 完成生产配置、类型安全配置、HTTP 超时、Actuator/OpenAPI 和 10 个后端测试；下一任务 T003 |
