...
T209 的本地实现已完成：The Odds 外部联赛/球队身份不再新建竞彩实体，赛事复核可选择性地组合确认联赛、主队与客队。下一步必须先在独立 PostgreSQL 环境审计真实 `NAME_CANDIDATE` 引用图，验证 V17 的受控清理和条件更新；在此证据补齐前，M2 保持 `PARTIAL`，T106/T107 不启动连续观测。T401～T405、T501～T507、T602、T603、T606、T607 均已完成；M4/M5 已完成，M6 保持 `PARTIAL`。
...
本地完成：T208 时效修正、T209 外部待复核身份与赛事组合复核
下一步：独立 PostgreSQL 验证 T208/T209 后，才启动 T106/T107 连续观测 -> T108 数据源 Go / No-Go 决策；T604 保持阻塞
```
