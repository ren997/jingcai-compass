...
| M3 预测发布闭环 | `PARTIAL` | T301～T304、T601 已完成；下一步实现公开预测快照 |
...
当前无活动任务。T304 已通过本地普通测试和 GitHub Actions PostgreSQL 16 集成验证并收口为 `DONE`。

下一主线任务为 `T305 公开预测快照`；其依赖的 T303 预测发布已完成，T304 也已补齐到期锁定和数据库不可变保护。
