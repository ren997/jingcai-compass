...
| M2 标准化与映射 | `PARTIAL` | T201～T208 的联赛、球队独立复核已完成；比赛映射复核时效修正与 T209 的外部待复核身份/自动复用修正待收口，未确认时不得由单场比赛映射反推别名 |
...
- 执行步骤：
  - [ ] 新增 Flyway migration，使 `provider_league_mappings.league_id`、`provider_team_mappings.team_id` 在 `PENDING`/`REJECTED` 外部身份阶段可为空；保留已确认关系的外键、状态约束、审计及唯一性。
  - [ ] 调整联赛与球队标准化：The Odds 新外部身份只保存 Provider、外部 ID、展示名、规范化键、`sport_key` 作用域与 `PENDING`，不在 `leagues`/`teams` 创建同名临时实体；仅确认到已存在的竞彩内部实体时写入实体 ID 与 `MANUAL_CONFIRMED`。
  - [ ] 审计并安全处理既有 The Odds `NAME_CANDIDATE` 待复核记录：先解除其临时实体关联；仅删除没有比赛、别名、已确认映射或其他引用的孤立临时实体，任何不满足删除条件的记录保留且标注为待人工处置，禁止从单场赛事关联反推或批量确认。
  - [ ] 将球队候选限定为可信的竞彩内部标准实体，排除未确认 Provider 临时实体；页面把“暂存实体”改为“已关联内部实体”，未确认时明确显示“尚未关联”。
  - [ ] 保持全局 `team_aliases` 不增加联赛字段：The Odds 无球队 ID 时，以现有 `provider_code + sport_key + 外部队名规范化键` 的 Provider 作用域身份作为复用键；同名队伍跨 `sport_key` 必须独立确认。
  - [ ] 不让单场赛事确认直接写入联赛/球队别名或 Provider 映射；可展示待确认建议，但联赛、主队、客队须分别由管理员确认并追加审计。
  - [ ] 当同一 Provider 事件的联赛、主队、客队均已有已确认映射，且作用域、主客方向、开赛时间阈值和事件唯一性全部满足时，自动确认赛事映射并允许同步亚盘；任一条件不满足时继续 `PENDING` 并保留可读原因。
...
- 完成标准：
  ...
  - V17 迁移、孤立临时实体处理、映射状态机和自动复用均经 PostgreSQL 16 CI 验证；本任务完成前 T106/T107 不启动连续观测。
...
下一步：完成 T208 时效修正与 T209 外部待复核身份/自动复用修正，再启动 T106/T107 连续观测 -> T108 数据源 Go / No-Go 决策；T604 保持阻塞
```
