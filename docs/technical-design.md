...
后端调用本机或受控模型命令入口属于可替换输入适配器，待模型运行方式确定后再启用，不作为 T302 完成前提。不在 P0 引入长期在线 Python 推理服务。无论输入来源如何，模型输出必须通过同一个明确的 `PredictionImportDto` 进入系统并校验：
...
- 每种 `MarketTypeEnum` 使用独立结算器并有参数化测试。
...
由 T006 使用托管 CI Runner、远程 Testcontainers PostgreSQL 或独立临时测试容器覆盖；不要求开发机安装 Docker，也不允许连接共享云端开发库：
...
1. T000～T005 已完成工程、配置、公共后端和前端测试基线。
2. 保留 T003 `SKIPPED` 记录，不要求开发机安装 Docker。
3. 执行 T006，在 CI 或远程临时 PostgreSQL 从空库运行全部 migration 和完整应用上下文测试。
...
3. 实现比赛候选映射和置信度。
4. 实现低置信度后台复核。
5. 通过 T207 串通体彩同步、标准化回填、比赛映射和亚盘快照。

### M3 预测发布闭环

1. 建预测和快照表。
2. 实现模型结果导入校验。
3. 提前完成管理员鉴权，确保预测发布 API 有真实安全边界。
4. 实现发布、锁定和哈希。
5. 生成公开快照。

### M4 赛果与结算

1. 建比赛事实版本和结算表，复用既有审计表。
2. 纯函数市场结算器可在数据库任务之前或同时实现。
3. 实现赛果同步和事实版本化。
4. 实现自动结算、异常和重算版本。

### M5 公共页面

1. 先完成数据库驱动的比赛列表和基础详情。
2. 增加公开预测详情与快照校验入口。
3. 完成全量历史和统计 API，再实现对应页面。
4. 最后汇总首页，不让首页包装阻塞纵向链路。
...
- PostgreSQL 集成测试使用 Testcontainers 或独立临时 PostgreSQL，不使用 H2；执行载体不要求开发机安装 Docker。
- 体彩与亚盘双 Provider。
