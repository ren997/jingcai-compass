...
function DraftCard({
  ...
  return <article className="admin-draft-card">
    ...
    <dl><div><dt>模型 / 特征</dt><dd>{item.modelVersion} / {item.featureVersion}</dd></div>
      ...
      <div><dt>让球倾向</dt><dd>{handicapPickLabel(item.handicapPick)}</dd></div><div><dt>预期总进球 / 置信度</dt><dd>{formatNumber(item.expectedTotalGoals)} / {confidenceLabel(item.confidenceLevel)}</dd></div>
